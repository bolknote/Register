...
- Интеграция не заменяет существующий блог: HTML, RSS, canonical URL, локальные комментарии и реакции продолжают работать независимо. В ActivityPub есть глобальная политика для будущих сохранений (записи/страницы, Article/Note, full/excerpt, Public/Unlisted), а редактор каждого материала может безопасно её переопределить. Смена defaults не запускает массовую рассылку, Update или Delete.
...
- Финальная локальная контрольная серия чистая: 788 unit-тестов / 10 862 assertions, 216 integration-тестов / 2 297 assertions, 58 acceptance-тестов / 629 assertions, lint 1 121 PHP-файла, PHPStan, Psalm, Phan, PHPMD, Rector, PHP 8.3–8.5 compatibility, Composer audit и dependency analysis. Production-сборка shared-hosting ZIP также проверена: в ней есть модуль, stable `phpseclib/phpseclib` 3.0.56, recovery/interoperability tools и runbooks, а dev-зависимостей нет.
