...
final readonly class DynamicSecretStore
{
    ...
    private function read(): array
    {
        ...
        $data = register_call_without_warnings(
    ...
    private function withExclusiveLock(\Closure $callback): mixed
    {
        ...
        $lock = register_call_without_warnings(static fn() => fopen($lockFilename, 'c+b'));
        if ($lock === false) {
    ...
    private function write(array $secrets): void
    {
        ...
        $handle = register_call_without_warnings(static fn() => fopen($temporaryFile, 'xb'));
        if ($handle === false) {
