...
final readonly class LinkInventoryRepository
{
    ...
    public function dueTargetIds(int $now, int $limit): array
    {
        ...
        $rows = $this->dbLayer
            ...
            ->andWhere('health_status NOT IN (:ignored, :blocked)')
            ...
            ->andWhere('(next_check_at <= :now OR (health_status = :broken AND next_check_at IS NULL'
                . ' AND last_checked_at IS NOT NULL AND last_checked_at <= :legacy_broken_before))')
            ->setParameter('now', $now)
            ->setParameter('broken', LinkHealthStatus::BROKEN->value)
            ->setParameter('legacy_broken_before', $now - LinkHealthPolicy::BROKEN_INTERVAL)
