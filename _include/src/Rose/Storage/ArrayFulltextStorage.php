...
{
    public const string PREFIX_KEYWORD = 'K';

    public const string PREFIX_TITLE   = 'T';
    ...
    /** @param array<int|string, array<int, int|string>> $fulltextIndex */
            ...
            if (\is_int($entries)) {
                ...
                foreach ($entries as $position) {
                    if ($position[0] === self::PREFIX_TITLE) {
                        $result[$id][self::TYPE_TITLE][] = (int)base_convert(substr($position, 1), 36, 10);
                    } elseif ($position[0] === self::PREFIX_KEYWORD) {
                        $result[$id][self::TYPE_KEYWORD][] = (int)base_convert(substr($position, 1), 36, 10);
                    } else {
                        $result[$id][self::TYPE_CONTENT][] = (int)base_convert($position, 36, 10);
                    }
                }
            }
        }
    ...
    public function addWord(string $word, int $id, int $type, int $position): void
    {
        ...
        if (isset($this->fulltextIndex[$word][$id])) {
            $positionStr = base_convert((string)$position, 10, 36);
            ...
            if (\is_int($value)) {
                ...
                $this->fulltextIndex[$word][$id] = base_convert((string)$value, 10, 36) . '|' . $positionStr;
            } else {
                // Appending
            ...
            if ($type === self::TYPE_KEYWORD) {
                ...
                $position = self::PREFIX_KEYWORD . base_convert((string)$position, 10, 36);
            } elseif ($type === self::TYPE_TITLE) {
                /** @noinspection CallableParameterUseCaseInTypeContextInspection */
                $position = self::PREFIX_TITLE . base_convert((string)$position, 10, 36);
            }
