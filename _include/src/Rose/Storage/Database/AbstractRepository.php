...
abstract class AbstractRepository
{
    ...
    public const string FULLTEXT_INDEX         = 'fulltext_index';

    protected const array DEFAULT_TABLE_NAMES     = [
        self::TOC                    => 'toc',
        self::WORD                   => 'word',
    ...
    protected const string POSITION_PREFIX_KEYWORD = 'k';

    protected const string POSITION_PREFIX_TITLE   = 't';
     ...
     */
    public function __construct(protected \PDO $pdo, string $prefix = '', array $options = [])
    {
        $this->assertValidIdentifier($prefix, 'prefix');

        $options = array_merge(self::DEFAULT_TABLE_NAMES, $options);
        array_walk($options, function (string $value, string $key): void {
     ...
     */
    public static function create(\PDO $pdo, string $prefix, array $options = []): self
    {
        $driverName = $pdo->getAttribute(\PDO::ATTR_DRIVER_NAME);
        if (!\is_string($driverName)) {
            throw new InvalidEnvironmentException('Database driver name must be a string.');
        }

        return match ($driverName) {
            'mysql' => new MysqlRepository($pdo, $prefix, $options),
            'pgsql' => new PostgresRepository($pdo, $prefix, $options),
            'sqlite' => new SqliteRepository($pdo, $prefix, $options),
    ...
    /**
     * @return array{bytes: int|float, rows: int|float}
     */
    ...
    protected function prepareStatement(string $sql): \PDOStatement
    {
        ...
        if (!$statement instanceof \PDOStatement) {
            throw new UnknownException('Database failed to prepare a statement.');
        }

        return $statement;
    }

    protected function queryStatement(string $sql): \PDOStatement
    {
        $statement = $this->pdo->query($sql);
        if (!$statement instanceof \PDOStatement) {
            throw new UnknownException('Database query failed.');
        }
    ...
    {
        return iterator_to_array($this->iterateAssociativeRows($statement), false);
    }

    /** @return \Generator<int, array<string, mixed>> */
    private function iterateAssociativeRows(\PDOStatement $statement): \Generator
    {
    ...
    /**
     * @param array<int|string, string> $titleWords
     * @param array<int|string, string> $keywords
     * @param array<int|string, string> $contentWords
     * @param array<int|string, int>    $wordIds
     *
     * @throws UnknownException
     */
    ...
    public function findFulltextByWords(array $words, ?int $instanceId = null): \Generator
    {
        ...
        try {
            ...
            if ($this->isUnknownTableException($e)) {
                throw new EmptyIndexException('There are no storage tables in the database. Call ' . self::class . '::erase() first.', 0, $e);
            }

            if ($this->isUnknownColumnException($e)) {
                throw new EmptyIndexException('There are outdated tables in the database. Call ' . self::class . '::erase() first.', 0, $e);
            }
        ...
        while ($row = $statement->fetch(\PDO::FETCH_ASSOC)) {
            ...
            if (!\is_string($positions)) {
                throw new UnknownException('Fulltext positions must be stored as a string.');
            }

            $row['title_positions'] = $row['keyword_positions'] = $row['content_positions'] = [];
            foreach (explode(',', $positions) as $positionWithPrefix) {
    ...
    {
        $st = $this->prepareStatement('INSERT INTO ' . $this->getTableName(self::METADATA) . ' (toc_id, word_count, images) VALUES (?, ?, ?)');
        try {
    ...
    {
        $data = array_map(fn(SnippetSource $s): string => $s->getMinPosition() . ','
            . $s->getMaxPosition() . ','
    ...
    public function removeFromIndex(ExternalId $externalId): void
    {
        ...
        try {
            $st = $this->prepareStatement("DELETE FROM {$this->getTableName(self::FULLTEXT_INDEX)} WHERE toc_id = ?");
            $st->execute([$tocId]);

            $st = $this->prepareStatement("DELETE FROM {$this->getTableName(self::METADATA)} WHERE toc_id = ?");
            $st->execute([$tocId]);

            $st = $this->prepareStatement("DELETE FROM {$this->getTableName(self::SNIPPET)} WHERE toc_id = ?");
            $st->execute([$tocId]);
        } catch (\PDOException $e) {
            if ($this->isUnknownTableException($e)) {
                throw new EmptyIndexException('There are missing storage tables in the database. Is ' . self::class . '::erase() running in another process?', 0, $e);
            }
    ...
    public function getTocEntries(array $criteria = []): array
    {
        ...
        return $this->fetchAssociativeRows($statement);
    }
    ...
    public function selectInternalIds(ExternalId ...$externalIds): array
    {
        ...
        $result = [];
        foreach ($data as $row) {
            $result[$this->getExternalIdFromRow($row)->toString()] = (int)$row['id'];
        }
