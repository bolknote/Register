...
{
    private const int KEYLEN         = 255;

    private const int UTF8MB4_KEYLEN = 191;
    ...
    {
        $charset = $this->queryStatement('SELECT @@character_set_connection')->fetchColumn();
        if ($charset !== 'utf8mb4') {
            $charset = 'utf8';
        }
    ...
    public function insertWords(array $words): void
    {
        ...
        if (\count($partWords) === 0) {
            return;
        }

        $sql = 'INSERT IGNORE INTO ' . $this->getTableName(self::WORD)
            . ' (name) VALUES ' . implode(',', array_fill(0, \count($partWords), '(?)'));

        try {
            $statement = $this->prepareStatement($sql);
            $statement->execute(array_values($partWords));
        } catch (\PDOException $e) {
            if ($this->isLockWaitingException($e)) {
...
LIMIT :limit";
    ...
    public function getIndexStat(): array
    {
        ...
        if (\count($tableStatuses) !== \count($tableNames)) {
            throw new EmptyIndexException('Some storage tables are missed in the database. Call ' . self::class . '::erase() first.');
        }

        $indexSize = 0;
        $indexRows = 0;
        foreach ($tableStatuses as $tableStatus) {
            $indexSize += (int)$tableStatus['Data_length'] + (int)$tableStatus['Index_length'];
            $indexRows += (int)$tableStatus['Rows'];
        }
    ...
    {
        return 1205 === (int)($e->errorInfo[1] ?? 0);
    }
