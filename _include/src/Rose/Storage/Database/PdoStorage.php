...
class PdoStorage implements StorageWriteInterface, StorageReadInterface, StorageEraseInterface, TransactionalStorageInterface
{
     ...
     */
    #[\Override]
    public function fulltextResultByWords(array $words, ?int $instanceId = null): FulltextIndexContent
    {
     ...
     */
    public function getTocByTitlePrefix($titlePrefix): array
    {
     ...
     * @throws UnknownIdException
     */
    private function getInternalIdFromExternalId(ExternalId $externalId): int
    {
        $id = $this->mapping->get($externalId);
        if ($id === null) {
            throw UnknownIdException::createIndexMissingExternalId($externalId);
        }
