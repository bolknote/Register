...
This directory contains Register's first-party search engine implementation. It provides
full-text indexing, English and Russian morphology, highlighting, snippets, and related-content
search. The code is autoloaded directly from `_include/src/Rose`; Register does not require an
external `s2/rose` Composer package, repository, or fork.

The implementation originated in Rose by Roman Parpalak and remains available under the MIT
license in [LICENSE](LICENSE). Register is the source of truth for this copy and its changes.

## Requirements

Rose follows Register's runtime requirement of PHP 8.3 or later. Database-backed indexes support
MySQL/MariaDB, PostgreSQL, and SQLite. A single-file storage is also available for small indexes.

No separate installation step is needed. Register's root Composer autoloader maps the `S2\Rose`
namespace to this directory.
...
Dictionary analyzers may return several normal forms for an ambiguous word. Implement
`WordNormalizerInterface` in this case; Rose indexes and searches every returned form at the
same logical word position. The interface extends `StemmerInterface`, so the analyzer remains
compatible with code which expects a regular stemmer.
